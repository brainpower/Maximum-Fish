...
CreatureIOPlugin::ObjPtr CreatureIOPlugin::loadObject(const boost::property_tree::ptree::value_type &node)
{
	...
	try
	{
		...
		re->curAgeFactor  = pt.get<float>("curAgeFactor");
...
CreatureIOPlugin::ObjPtr CreatureIOPlugin::loadObject(const boost::property_tree::ptree::value_type &node, const std::shared_ptr<SimState> &s)
{
	...
	try
	{
		...
		re.reset( new Creature( Simulator::GetSpecies(pt.get<std::string>("species"), s) ) );
...
bool CreatureIOPlugin::saveObject( const std::string& name, const Creature &c, boost::property_tree::ptree &r)
{
	try
	{
		...
		pt.put<float>("curAgeFactor", c.curAgeFactor);
